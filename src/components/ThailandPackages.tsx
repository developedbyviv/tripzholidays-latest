...
const packages = [
  {
    ...
    duration: 6,
    nights: 5,
    title: 'Thailand Island Hopping | Phuket, Krabi & Phi Phi Islands',
    originalPrice: 'INR 1,65,900',
    discountedPrice: 'INR 1,32,000',
    rating: '4.8',
    reviews: '(67)',
  },
  {
  ...
  },
  {
    ...
    duration: 10,
    nights: 9,
    title: 'Thailand Grand Tour | Culture, Nature & Beach Paradise',
    originalPrice: 'INR 2,45,900',
    discountedPrice: 'INR 2,01,000',
    rating: '4.9',
    reviews: '(124)',
  },
];
...
export default function ThailandPackages() {
  const handleViewAll = () => {
    alert('View all Thailand packages!');
  };

  const handleViewItinerary = (id: number) => {
    alert(`View itinerary for Thailand package ${id}`);
  };

  const handleCall = (id: number) => {
    alert(`Call us for Thailand package ${id}\nPhone: +91-9876543210`);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 -gap-1">
          {packages.map((pkg) => (
            <PackageCard
              key={pkg.id}
              package={pkg}
              tags="Islands Beaches Culture Temples Adventure"
